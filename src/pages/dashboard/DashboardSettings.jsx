...
const Textarea = ({ value, onChange, maxLength, rows = 3, placeholder }) => (
  <textarea
    value={value} onChange={onChange} maxLength={maxLength} rows={rows} placeholder={placeholder}
    className="w-full rounded-xl border border-border bg-background px-3.5 py-2.5
               text-sm text-foreground placeholder:text-muted-foreground/60
               focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary
...
const InlineForm = ({ open, children }) => (
  <AnimatePresence>
    {open && (
      ...
      >
        <div className="pb-4 pt-1 space-y-3 border-b border-border/50">
          {children}
        </div>
      </motion.div>
    )}
...
const ProfileTab = ({ user, updateProfile, queryClient, navigate }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError("Failed to save profile."); setSaving(false)
    }
  }
...
const AccountTab = ({ user }) => {
  ...
  const resetForms = () => {
    ...
    setError(null); setSuccess(null); setShowPass(false); setShowNew(false)
  }
  ...
  const handlePasswordSubmit = async (e) => {
    e.preventDefault()
    if (!password || !newPassword || !confirmPass) { setError("All fields required."); return }
  ...
  return (
    <div>
      {success && (
        <div className="mb-4 px-4 py-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-sm text-emerald-500 flex items-center gap-2">
...
const AcademicTab = ({ user, completeAcademicProfile, queryClient }) => {
  ...
  useEffect(() => {
    ...
    if (prev !== programId && programId !== (user?.programId || "")) {
      setBranchId("")
    }
  }, [programId])
    ...
    <form onSubmit={handleSubmit}>
      ...
      <Section title="Your Institution">
        <div className="py-3.5 border-b border-border/50 space-y-1.5">
          ...
          <Dropdown value={universityId} onChange={setUniversityId}
        ...
        <div className="py-3.5 border-b border-border/50 space-y-1.5">
          <label className="flex items-center gap-1.5 text-xs font-medium text-muted-foreground uppercase tracking-wide">
            <BookOpen size={11} /> Program
            {programsLoading && <Loader2 size={10} className="animate-spin ml-1" />}
          </label>
          <Dropdown value={programId} onChange={setProgramId}
            options={programs.map(p => ({ value: p.$id, label: p.name }))}
            disabled={!universityId || programsLoading}
            placeholder={programsLoading ? "Fetching programs…" : "Select program"} />
        </div>
        ...
        <div className="py-3.5 space-y-1.5">
          ...
          <Dropdown value={branchId} onChange={setBranchId}
            ...
            placeholder={branchesLoading ? "Fetching branches…" : "Select branch"} />
        </div>
      </Section>
