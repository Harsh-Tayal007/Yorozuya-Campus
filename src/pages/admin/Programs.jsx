...
import { BookOpen, Plus, Pencil, Trash2, X, Check, Loader2, AlertTriangle } from "lucide-react"
...
// ── Program card ──────────────────────────────────────────────────────────────
function ProgramCard({ program, uniName, onEdit, onDelete }) {
  return (
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          <div className="flex flex-wrap gap-x-3 gap-y-0.5 ml-9">
            {program.degreeType && (
              <span className="text-[11px] text-muted-foreground">{program.degreeType}</span>
            )}
            {program.duration && (
              <span className="text-[11px] text-muted-foreground">
                {String(program.duration).includes("year") ? program.duration : `${program.duration} years`}
              </span>
            )}
            {uniName && (
              <span className="text-[11px] text-muted-foreground/60 truncate">{uniName}</span>
            )}
          </div>
        </div>
...
const Programs = () => {
  ...
  const [form, setForm] = useState({ name: "", slug: "", universityId: "", degreeType: "", duration: "" })
  ...
  const uniMap = useMemo(() => {
    const m = {}; universities.forEach(u => { m[u.$id] = u.name }); return m
  }, [universities])

  const visiblePrograms = useMemo(() =>
    filterUni === "all" ? programs : programs.filter(p => p.universityId === filterUni),
    [programs, filterUni])
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => updateProgram(id, data, user),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["programs-all"] })
  ...
  const deleteMutation = useMutation({
    mutationFn: ({ id, name }) => deleteProgram(id, user, name),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["programs-all"] })
  ...
  const resetForm = () => {
    setForm({ name: "", slug: "", universityId: "", degreeType: "", duration: "" })
    setEditingProgram(null)
  }

  const handleSubmit = (e) => {
    e.preventDefault()
    if (!form.name || !form.universityId) { toast.error("Name and university are required"); return }
    if (editingProgram) updateMutation.mutate({ id: editingProgram.$id, data: form })
    else createMutation.mutate(form)
  }

  const handleEdit = (program) => {
    setEditingProgram(program)
    setForm({ name: program.name, slug: program.slug, universityId: program.universityId,
    ...
    <div className="space-y-8 max-w-7xl">
      ...
      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">Manage academic programs across universities</p>
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
              ...
              <form onSubmit={handleSubmit} className="space-y-2.5">
                ...
                <input value={form.slug} onChange={e => updateForm("slug", e.target.value)}
                  ...
                  className="w-full h-10 px-3.5 rounded-xl text-sm border border-border/60 bg-card/80
                             ...
                             focus:outline-none focus:ring-2 focus:ring-violet-500/25 focus:border-violet-500/60
                             hover:border-border transition-all duration-150" />
                <CustomSelect value={form.universityId} onChange={v => updateForm("universityId", v)}
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}
          ...
          <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
            ...
            <div className="w-48">
              <CustomSelect value={filterUni} onChange={setFilterUni}
                placeholder="All Universities"
                options={[{ value: "all", label: "All Universities" }, ...universities.map(u => ({ value: u.$id, label: u.name }))]} />
            </div>
          </div>
            ...
            <AnimatePresence mode="popLayout">
              <div className="space-y-2.5">
                {visiblePrograms.map(program => (
                  <ProgramCard key={program.$id} program={program} uniName={uniMap[program.universityId]}
