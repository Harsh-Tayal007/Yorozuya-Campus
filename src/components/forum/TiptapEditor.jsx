...
// Saves content as HTML. No raw markdown symbols visible while typing.
//
// Props:
  ...
  return createPortal(
    ...
    >
      ...
      <div style={{
        ...
        borderRadius: 14, padding: 20, width: 320,
        boxShadow: "0 20px 60px rgba(0,0,0,0.5)",
        animation: "ttSlideUp 0.18s ease",
      }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          <div>
            ...
            <input autoFocus value={text} onChange={e => setText(e.target.value)} placeholder="Link text (optional)"
              style={{
          ...
          <div>
            ...
            <input value={url} onChange={e => setUrl(e.target.value)}
              onKeyDown={e => e.key === "Enter" && handleSave()} placeholder="https://..."
              style={{
                width:"100%", fontSize:13, padding:"8px 12px", borderRadius:8,
                border:"1px solid var(--border)", background:"rgba(255,255,255,0.04)",
                color:"var(--foreground)", outline:"none", boxSizing:"border-box",
...
function Tooltip({ label, targetRef, visible }) {
  ...
  return createPortal(
    <div style={{
      ...
      animation:"ttTipIn 0.12s ease forwards",
    }}>
      <div style={{
        background:"rgba(10,10,16,0.95)", backdropFilter:"blur(10px)",
        border:"1px solid rgba(255,255,255,0.12)", borderRadius:8,
        padding:"4px 10px", fontSize:11, fontWeight:500,
        color:"rgba(255,255,255,0.95)", letterSpacing:"0.01em",
        boxShadow:"0 6px 20px rgba(0,0,0,0.4)", whiteSpace:"nowrap", marginBottom:6,
      }}>{label}</div>
      <div style={{
        position:"absolute", bottom:2, left:"50%",
        transform:"translateX(-50%) rotate(45deg)",
        width:6, height:6, background:"rgba(10,10,16,0.95)",
    ...
    <>
      <button
        ...
        style={{
          ...
          background: active ? "rgba(59,130,246,0.15)" : hovered ? "rgba(255,255,255,0.08)" : "none",
          color: active ? "#3b82f6" : hovered ? "var(--foreground)" : "var(--muted-foreground)",
...
/* ─── Main Editor ────────────────────────────────────────────── */
export default function TiptapEditor({ content, onChange, onSubmit, placeholder, autoFocus }) {
  ...
  // Sync external content reset (e.g. after submit)
  useEffect(() => {
    if (!editor) return
    ...
    <>
      <style>{`
        ...
        .tiptap-editor .ProseMirror pre code {
          ...
          color: var(--foreground); font-size: 0.82em;
        }
      ...
      <div style={{
        ...
        <ToolbarBtn title="Insert link" active={editor.isActive("link")} onClick={openLinkModal}>
          <LinkIcon size={13} />
        </ToolbarBtn>
