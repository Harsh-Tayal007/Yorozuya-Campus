...
export default function CreateReplyBox() {
        ...
        if (cs.uploadedImagePublicId) {
            try {
                await functions.createExecution(
                    CLOUDINARY_FN,
                    JSON.stringify({
                        publicId: cs.uploadedImagePublicId
                    })
                );
            } catch (err) {
    ...
    `}</style>
            ...
            <div className="rounded-xl border border-border bg-muted/20 overflow-hidden">
                ...
                {!isExpanded ? (
                    <button
                        onClick={() => { setIsExpanded(true); setTimeout(() => taRef.current?.focus(), 50) }}
                    ...
                    <div className="reply-expand flex flex-col">
                        ...
                        <Textarea
                            ...
                            className="resize-none border-0 border-b border-border rounded-none
  ...
  <div className="relative inline-flex">
                                {isUploading ? (
                                    ...
                                    <>
                                        <img src={cs.previewImage} alt="image preview" className="max-h-24 max-w-[160px] rounded-lg border border-border object-cover" />
                                        <button onClick={removeImage} className="absolute top-1.5 right-1.5 w-5 h-5 rounded-full bg-background border border-border flex items-center justify-center text-[9px] text-muted-foreground hover:text-destructive transition-colors">✕</button>
                                    </>
                                )}
                                </div>
                            </div>
                        ...
                        {cs.previewGif && (
                            <div className="flex px-4 pt-3">
                                <div className="relative inline-flex">
                                    ...
                                    <button onClick={removeGif} className="absolute top-1.5 right-1.5 w-5 h-5 rounded-full bg-background border border-border flex items-center justify-center text-[9px] text-muted-foreground hover:text-destructive transition-colors">✕</button>
                                </div>
                            </div>
                        )}
                        ...
                        <div className="flex items-center justify-between px-3 py-2">
                           ...
                           transition-[color,background-color,transform] duration-150"
                                ...
                                <button
                                    ...
                                    onClick={() => { cs.setGifOpen(v => !v); taRef.current?.blur() }}
                           ...
                           transition-[color,background-color,transform] duration-150"
                            ...
                            <div className="flex gap-2">
                                <Button size="sm" variant="ghost" className="h-7 px-3 rounded-full text-xs" onClick={resetCompose}>
                                    Cancel
                                </Button>
                                <Button
                                    size="sm"
                                    ...
                                    disabled={isPosting || isUploading || (!cs.text.trim() && !cs.previewGif && !cs.previewImage)}
                                >
