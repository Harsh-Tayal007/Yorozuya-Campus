...
export default function CreateReplyBox() {
  ...
  const handleSubmit = async () => {
    ...
    await createReply.mutateAsync({
      threadId, content: html, gifUrl: cs.previewGif,
      imageUrl: cs.previewImage, imagePublicId: cs.uploadedImagePublicId,
      authorId: user.$id, authorName: user.username, parentReplyId: null,
    })
    setHtml("")
    cs.setPreviewGif(null); cs.setPreviewImage(null); cs.setUploadedImagePublicId(null)
    cs.setGifOpen(false); cs.setGifSearch(""); setIsExpanded(false)
  }
  ...
  const resetCompose = async () => {
    ...
    cs.setPreviewImage(null); cs.setUploadedImagePublicId(null)
    cs.setGifOpen(false); cs.setGifSearch(""); setIsExpanded(false)
  }
  ...
  const handleImageSelect = async (e) => {
    ...
    try {
      ...
      cs.setPreviewImage(url); cs.setUploadedImagePublicId(publicId)
    } catch (err) {
      ...
      <div className="rounded-xl border border-border bg-muted/20 overflow-hidden">
        ...
        {!isExpanded ? (
          ...
          <div className="reply-expand flex flex-col">
            <TiptapEditor content={html} onChange={setHtml} onSubmit={handleSubmit} placeholder="Write a reply…" autoFocus />
                    ...
                    <>
                      ...
                      <button onClick={removeImage}
                        className="absolute top-1.5 right-1.5 w-5 h-5 rounded-full bg-background
                                   ...
                                   text-[9px] text-muted-foreground hover:text-destructive transition-colors">✕</button>
                    </>
                  )}
                </div>
              </div>
            ...
            {cs.gifOpen && (
              <div className="px-4 pt-3">
                <GifPicker colors={cs.colors} gifSearch={cs.gifSearch} setGifSearch={cs.setGifSearch}
                  ...
                  onSelectGif={async (url) => {
                    ...
                    cs.setPreviewGif(url); cs.setGifOpen(false)
                  }} />
              </div>
            )}
            <div className="flex items-center justify-between px-3 py-2">
              <div className="flex items-center gap-1 text-muted-foreground">
                <button disabled={isUploading || !!cs.previewGif} onClick={() => { cs.setPreviewGif(null); fileRef.current.click() }}
                ...
                <button disabled={!!cs.previewImage} onClick={() => cs.setGifOpen(v => !v)}
                  className="flex items-center px-2.5 py-1.5 rounded-full text-xs font-bold
                             hover:bg-muted hover:text-foreground active:scale-95
                             disabled:opacity-50 disabled:cursor-not-allowed
                             transition-[color,background-color,transform] duration-150">GIF</button>
              </div>
              <div className="flex gap-2">
                <Button size="sm" variant="ghost" className="h-7 px-3 rounded-full text-xs" onClick={resetCompose}>Cancel</Button>
                <Button size="sm" className="h-7 px-3 rounded-full text-xs" onClick={handleSubmit} disabled={isPosting || isUploading || !canPost}>
