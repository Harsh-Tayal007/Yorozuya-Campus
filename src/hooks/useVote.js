...
import {
  computeKarmaDelta,
  updateKarmaForVote,
} from "@/services/user/karmaService";
...
export default function useVote(
  ...
  useEffect(() => {
    setVoteDocId(initialVoteDocId);
  }, [initialVoteDocId]);

  const [seedLocked, setSeedLocked] = useState(
    () => initialVote === "up" && initialVoteDocId === null,
  );

  useEffect(() => {
    if (!seedLocked) return;
    const t = setTimeout(() => setSeedLocked(false), 2000);
  ...
  return { vote, score, upvotes, downvotes, handleVote, loading: loading || seedLocked }
}
