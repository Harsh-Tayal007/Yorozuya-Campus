...
export async function enrollStudent({ classId, studentId, studentName, rollNumber, isLeet }) {
...
export async function updateClass(classId, updates) {
  return databases.updateDocument(
    DATABASE_ID, CLASSES_COLLECTION_ID, classId, updates
  )
}

export async function deleteClassAndAllData(classId) {
  const { DATABASE_ID: DB, SESSIONS_COLLECTION_ID, ATTENDANCE_RECORDS_COLLECTION_ID, SESSION_TOKENS_COLLECTION_ID } = await import("@/config/appwrite")
  ...
  await Promise.all(sessionRes.documents.map(async s => {
    ...
    await Promise.all([
      ...
      ...tokens.documents.map(t =>
        databases.deleteDocument(DATABASE_ID, SESSION_TOKENS_COLLECTION_ID, t.$id)),
